//============================================================================
// microdoppler.hpp -- telling a drone from a bird from a car
//
...
// line sits in the middle of a wide, two-sided skirt, with a sharp flash every
// time a blade passes broadside.  A walking person's limbs swing forwards
// faster than they swing back, giving a narrower, lopsided smear.  A car is a
// rigid box and gives one clean line and almost nothing else.
//
...
// defended in a report.  It also degrades honestly: when the evidence does not
// match any of the three the answer is "unknown", not the nearest neighbour in
// some feature space nobody can inspect.
//
// The transform is a short-time one, because the point is precisely that the
// spectrum changes within a single coherent interval.  A blade flash lasting a
// millisecond is invisible in a transform over the whole thirty-two, and
// obvious in a sliding sixty-four-sample window.
//============================================================================
#pragma once
...
public:
    ...
    /// Drop the accumulated evidence for a track that has died, so its
    /// identifier can be reused without inheriting a stale opinion.
    ...
    double stft_rate_hz() const { return prf_hz_ / kHop; } ///< spectrogram frames/second
    double hz_per_ms()    const { return 2.0 / lambda_m_; }///< Doppler per metre/second

    static constexpr int kWin = 64;   ///< short-time window, samples
    static constexpr int kHop = 16;   ///< 75 per cent overlap
    static constexpr int kNotch = 2;  ///< bins either side of the bulk line
