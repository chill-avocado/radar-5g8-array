...
Config load_config(const std::string& path) {
    ...
    Config c;
    c.source          = source_from(j.get<std::string>("source", source_name(c.source)), c.source);
    c.device_args     = j.get<std::string>("device_args", c.device_args);
    c.centre_freq_hz  = j.get<double>("centre_freq_hz",  c.centre_freq_hz);
    c.sample_rate_hz  = j.get<double>("sample_rate_hz",  c.sample_rate_hz);
    c.tx_gain_db      = j.get<double>("tx_gain_db",      c.tx_gain_db);
    c.rx_gain_db      = j.get<double>("rx_gain_db",      c.rx_gain_db);
    c.rx_bandwidth_hz = j.get<double>("rx_bandwidth_hz", c.rx_bandwidth_hz);
    c.clock_source    = j.get<std::string>("clock_source", c.clock_source);
    c.time_source     = j.get<std::string>("time_source",  c.time_source);

    c.sweep_bw_hz = j.get<double>("sweep_bw_hz", c.sweep_bw_hz);
    c.n_sweep     = j.get<int>("n_sweep", c.n_sweep);
    c.n_pri       = j.get<int>("n_pri",   c.n_pri);
    c.n_chirp     = j.get<int>("n_chirp", c.n_chirp);
    c.mimo        = mimo_from(j.get<std::string>("mimo", mimo_name(c.mimo)), c.mimo);
    c.tx_enable   = j.get<bool>("tx_enable", c.tx_enable);

    c.decim        = j.get<int>("decim",       c.decim);
    c.n_range_fft  = j.get<int>("n_range_fft", c.n_range_fft);
    c.n_range      = j.get<int>("n_range",     c.n_range);
    c.n_doppler    = j.get<int>("n_doppler",   c.n_doppler);
    c.range_window = window_from(j.get<std::string>("range_window", window_name(c.range_window)), c.range_window);
    c.dopp_window  = window_from(j.get<std::string>("dopp_window",  window_name(c.dopp_window)),  c.dopp_window);

    c.cfar_kind       = cfar_from(j.get<std::string>("cfar_kind", cfar_name(c.cfar_kind)), c.cfar_kind);
    c.guard_range     = j.get<int>("guard_range",  c.guard_range);
    c.guard_dopp      = j.get<int>("guard_dopp",   c.guard_dopp);
    c.train_range     = j.get<int>("train_range",  c.train_range);
    c.train_dopp      = j.get<int>("train_dopp",   c.train_dopp);
    c.pfa             = j.get<double>("pfa",       c.pfa);
    c.max_hits        = j.get<int>("max_hits",     c.max_hits);
    c.zero_dopp_blank = j.get<int>("zero_dopp_blank", c.zero_dopp_blank);
    c.range_zero_bin  = j.get<int>("range_zero_bin",  c.range_zero_bin);

    c.aoa             = aoa_from(j.get<std::string>("aoa", aoa_name(c.aoa)), c.aoa);
    c.aoa_az_bins     = j.get<int>("aoa_az_bins",  c.aoa_az_bins);
    c.aoa_el_bins     = j.get<int>("aoa_el_bins",  c.aoa_el_bins);
    c.aoa_az_span_deg = j.get<double>("aoa_az_span_deg", c.aoa_az_span_deg);
    c.aoa_el_span_deg = j.get<double>("aoa_el_span_deg", c.aoa_el_span_deg);
    c.cluster_eps_m   = j.get<double>("cluster_eps_m",   c.cluster_eps_m);
    c.cluster_eps_ms  = j.get<double>("cluster_eps_ms",  c.cluster_eps_ms);
    c.cluster_eps_deg = j.get<double>("cluster_eps_deg", c.cluster_eps_deg);
    c.cluster_min_pts = j.get<int>("cluster_min_pts",    c.cluster_min_pts);
    c.track_gate_chi2 = j.get<double>("track_gate_chi2", c.track_gate_chi2);
    c.track_confirm_n = j.get<int>("track_confirm_n",    c.track_confirm_n);
    c.track_drop_n    = j.get<int>("track_drop_n",       c.track_drop_n);
    c.track_q_accel   = j.get<double>("track_q_accel",   c.track_q_accel);

    c.worker_threads = j.get<int>("worker_threads", c.worker_threads);
    c.realtime       = j.get<bool>("realtime",      c.realtime);
    c.http_port      = j.get<int>("http_port",      c.http_port);
    c.web_root       = j.get<std::string>("web_root",    c.web_root);
    c.record_path    = j.get<std::string>("record_path", c.record_path);
    c.calib_path     = j.get<std::string>("calib_path",  c.calib_path);
