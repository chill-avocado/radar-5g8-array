...
{hole_s}
    ...
    boxes = {
        'tx': [('tray', T[0], T[1], T[2], T[3]),
               ('arm', T[2] - OVL, y_split, lap1, bar_top),
               ('pad', pad_tx - PAD_L / 2, bar_top,
                pad_tx + PAD_L / 2, bar_top + PAD_D - HALF),
               ('gusset', T[2] - OVL, bar_top, T[2], T[1])],
        'rx': [('tray', R[0], R[1], R[2], R[3]),
               ('arm', lap0, bar_bot, R[0] + OVL, y_split),
               ('pad', pad_rx - PAD_L / 2, bar_bot - (PAD_D - HALF),
                pad_rx + PAD_L / 2, bar_bot),
               ('gusset', R[0], y_split, R[0] + OVL, R[1])],
        'holes': (tx_holes, rx_holes),
        # exactly the pcy that arm() drills, not a re-derivation of it
        'pads': ((pad_tx, y_split + PAD_D / 2.0),
                 (pad_rx, y_split - PAD_D / 2.0)),
    }
...
{tray(tx, T, 'tx')}
{tray(rx, R, 'rx')}
{arm('tx', T[2] - OVL, lap1, True, tx_holes, pad_tx)}
{arm('rx', lap0, R[0] + OVL, False, rx_holes, pad_rx)}
